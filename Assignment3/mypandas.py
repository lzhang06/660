...
class DataFrame(object):
        ...
        if not isinstance(reverse, bool ):
            ...
            def get_key(row):
                if isinstance(column_name, str):
                    ...
                    return tuple(row[c] for c in column_name)

            return sorted(self, key = get_key, reverse = reverse)
    ...
    def group_by(self, column_name1, column_name2, func):
        if isinstance(column_name1, str):
        ...
        elif isinstance(column_name1, list):
            inputs = self[column_name1 + [column_name2]]
            inputs_dict = dict()
            avgDict = dict()
            
            for each in inputs:
                group_key = tuple(i for i in each[:-1])
                if group_key in inputs_dict:
                    inputs_dict[group_key].append(each[-1])
                else:
                    inputs_dict[group_key] = [each[-1]] 
            for k, v in inputs_dict.items():
                avgDict[k] = func(v)
            return avgDict
